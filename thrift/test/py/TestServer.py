...
class TestContextHandler(ThriftTest.ContextIface):
  ...
  def testVoid(self, handler_ctx):
    ...
    if not handler_ctx[0].endswith("127.0.0.1") or handler_ctx[1] == self._server_port:
...
if __name__ == "__main__":
  ...
  if args[0] == "THttpServer":
    try:
      server = THttpServer.THttpServer(handler, ('', options.port), pfactory)
    except:
    ...
    if options.ssl:
      ...
      transport = TSSLSocket.TSSLServerSocket(options.port, certfile=cert_path)
    else:
    ...
    if args[0] == "TNonblockingServer":
      server = TNonblockingServer.TNonblockingServer(handler, transport,
                                                     pfactory,
                                                     readTimeout=options.timeout)
    elif args[0] == "TProcessPoolServer":
      server = TProcessPoolServer.TProcessPoolServer(handler, transport,
                                                     tfactory,
                                                     pfactory)
    else:
