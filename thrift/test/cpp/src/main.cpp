...
int main(int argc, char **argv) {
  ...
  if (runServer) {
    ...
    if (serverType == "simple") {
      ...
      serverThread = threadFactory->newThread(std::shared_ptr<TServer>(new TSimpleServer(serviceProcessor, serverSocket, transportFactory, protocolFactory)));
      
    } else if (serverType == "threaded") {

      serverThread = threadFactory->newThread(std::shared_ptr<TServer>(new TThreadedServer(serviceProcessor, serverSocket, transportFactory, protocolFactory)));
      ...
      threadManager->start();     
      serverThread = threadFactory->newThread(std::shared_ptr<TServer>(new TThreadPoolServer(serviceProcessor, serverSocket, transportFactory, protocolFactory, threadManager)));
    }
  ...
  if (clientCount > 0) {
    ...
    for (size_t ix = 0; ix < clientCount; ix++) {
      ...
      std::shared_ptr<TSocket> socket(new TSocket("127.0.01", port));
