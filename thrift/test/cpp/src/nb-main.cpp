...
public:
  ...
  ClientThread(std::shared_ptr<TTransport>transport, std::shared_ptr<ServiceClient> client, Monitor& monitor, size_t& workerCount, size_t loopCount, TType loopType) :
...
int main(int argc, char **argv) {
  ...
  if (runServer) {
    ...
    if (serverType == "simple") {
      ...
      serverThread = threadFactory->newThread(std::shared_ptr<TServer>(new TNonblockingServer(serviceProcessor, protocolFactory, port)));
      serverThread2 = threadFactory->newThread(std::shared_ptr<TServer>(new TNonblockingServer(serviceProcessor, protocolFactory, port+1)));
      ...
      threadManager->start();
      serverThread = threadFactory->newThread(std::shared_ptr<TServer>(new TNonblockingServer(serviceProcessor, protocolFactory, port, threadManager)));
      serverThread2 = threadFactory->newThread(std::shared_ptr<TServer>(new TNonblockingServer(serviceProcessor, protocolFactory, port+1, threadManager)));
    }
  ...
  if (clientCount > 0) {
    ...
    for (size_t ix = 0; ix < clientCount; ix++) {
      ...
      clientThreads.insert(threadFactory->newThread(std::shared_ptr<ClientThread>(new ClientThread(socket, serviceClient, monitor, threadCount, loopCount, loopType))));
    }

    for (std::set<std::shared_ptr<Thread> >::const_iterator thread = clientThreads.begin(); thread != clientThreads.end(); thread++) {
      (*thread)->start();
    }
    ...
    for (set<std::shared_ptr<Thread> >::iterator ix = clientThreads.begin(); ix != clientThreads.end(); ix++) {

      std::shared_ptr<ClientThread> client = std::dynamic_pointer_cast<ClientThread>((*ix)->runnable());
