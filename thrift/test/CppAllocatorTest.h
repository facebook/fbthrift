...
using PmrByteAlloc = pmr::polymorphic_allocator<std::byte>;
...
struct CountingPmrResource : public pmr::memory_resource {
 ...
 private:
  ...
  void* do_allocate(size_t bytes, size_t alignment) override {
    (*counter_)++;
    return pmr::get_default_resource()->allocate(bytes, alignment);
  }
  void do_deallocate(void* p, size_t bytes, size_t alignment) override {
    pmr::get_default_resource()->deallocate(p, bytes, alignment);
  }
