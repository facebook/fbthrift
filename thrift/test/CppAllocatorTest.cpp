...
TEST(CppAllocatorTest, UsesAllocatorPmr) {
  PmrByteAlloc alloc(pmr::null_memory_resource());
  ParentPmr s(alloc);
...
TEST(CppAllocatorTest, DeserializePmr) {
  ...
  serializer::deserialize(str, s2);
  EXPECT_EQ(s2.aa_list(), (pmr::vector<int32_t>{1, 2, 3}));
  EXPECT_EQ(s2.aa_set(), (pmr::set<int32_t>{1, 2, 3}));
  EXPECT_EQ(s2.aa_map(), (pmr::map<int32_t, int32_t>{{1, 1}, {2, 2}, {3, 3}}));
...
// FIXME DeserializeSortedUniqueConstructiblePmr ???
