...
object (self)
  ...
  method testMapMap x = p "testMapMap(%s)\n" (Int32.to_string (sod x));
    let mm = Hashtbl.create 3 in
    let pos = Hashtbl.create 7 in
    let neg = Hashtbl.create 7 in
      for i=1 to 4 do
        Hashtbl.add pos (Int32.of_int i) (Int32.of_int i);
        Hashtbl.add neg (Int32.of_int (-i)) (Int32.of_int (-i));
      done;
      Hashtbl.add mm (Int32.of_int 4) pos;
      Hashtbl.add mm (Int32.of_int (-4)) neg;
      mm
