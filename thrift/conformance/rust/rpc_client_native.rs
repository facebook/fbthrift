...
async fn get_client(
    ...
    port: u16,
) -> Result<Arc<dyn RPCConformanceServiceExt<nativeclient::NativeChannel> + Send + Sync + 'static>>
{
    let channel = nativeclient::NativeChannelBuilder::from_sock_addr(
        fb,
...
async fn test(client: &dyn RPCConformanceServiceExt<nativeclient::NativeChannel>) -> Result<()> {
...
async fn request_response_timeout(
    ...
    let mut opts = nativeclient::RpcOptions::default();
