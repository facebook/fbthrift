...
class TSSLSocket(TSocket):
  ...
  def _getCertNames(cert, includeAlt=None):
    ...
    # distinguished names (RDNs) given in the certificate's data structure for
    # the principal, and each RDN is a sequence of name-value pairs.
    names = set()
  ...
  def _matchName(name, pattern):
    """match a DNS name against a pattern. match is not case sensitive. a '*'
    in the pattern will match any single component of name."""
