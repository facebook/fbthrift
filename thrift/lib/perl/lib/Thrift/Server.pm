...
sub serve
{
    ...
    while (1)
    {
        my $oldTimeout = $self->{serverTransport}->timeout();
        $self->{serverTransport}->timeout(0.5);
        my $client;
        while(!$client) {
          $self->_collectChildren();
          $client = $self->{serverTransport}->accept();
        }
        $self->{serverTransport}->timeout($oldTimeout);
...
sub _client
{
    ...
    if($pid) {
      ...
      return $self->_handleException("fork() failed: $!");
    } else {
      eval {
        my $itrans = $self->{inputTransportFactory}->getTransport($client);
        my $otrans = $self->{outputTransportFactory}->getTransport($client);
