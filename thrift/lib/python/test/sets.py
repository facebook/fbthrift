...
class SetTests(unittest.TestCase):
    ...
    def test_typedef_isinstance(self) -> None:
        ...
        if not self.is_mutable_run:
            # Mutable sets cannot contain nested containers (e.g. set<list<i32>>)
            # because MutableList's internal data is a Python list, which is
            # unhashable and cannot be added to a Python set.
