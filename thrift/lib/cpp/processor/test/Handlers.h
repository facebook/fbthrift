...
class ChildHandler : public ParentHandler, virtual public ChildServiceIf {
 public:
  ChildHandler(const std::shared_ptr<EventLog>& log) :
      ParentHandler(log),
      value_(0) {}
...
class ServerEventHandler : public server::TServerEventHandler {
 public:
  ServerEventHandler(const std::shared_ptr<EventLog>& log) :
      nextId_(1),
      log_(log) {}
...
class ProcessorEventHandler : public TProcessorEventHandler {
 public:
  ProcessorEventHandler(const std::shared_ptr<EventLog>& log) :
      nextId_(1),
      log_(log) {}
