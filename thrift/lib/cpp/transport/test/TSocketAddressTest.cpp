...
TEST(SocketAddress, ConstructFromInvalidIpv4) {
  EXPECT_THROW(TSocketAddress("1.2.3.256", 1234), TTransportException);
}
...
TEST(SocketAddress, SetFromHostname) {
  ...
  EXPECT_THROW(TSocketAddress("localhost", 80), TTransportException);
  TSocketAddress addr("localhost", 80, true);

  TSocketAddress addr2;
  EXPECT_THROW(addr2.setFromIpPort("localhost", 0), TTransportException);
