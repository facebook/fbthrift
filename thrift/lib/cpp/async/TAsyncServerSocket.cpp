...
int TAsyncServerSocket::stopAccepting(int shutdownFlags) {
  int result = 0;
  for (auto& handler : sockets_) {
    T_DEBUG_L(1, "TAsyncServerSocket::stopAccepting(this=%p, fd=%d)", this,
              handler.socket_);
  }
...
void TAsyncServerSocket::useExistingSockets(const std::vector<int>& fds) {
  assert(eventBase_ == nullptr || eventBase_->isInEventBaseThread());
