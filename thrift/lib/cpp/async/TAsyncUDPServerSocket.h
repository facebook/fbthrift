 ...
 */
class TAsyncUDPServerSocket : private TAsyncUDPSocket::ReadCallback {
 public:
  class Callback {
   public:
    /**
   ...
   */
  explicit TAsyncUDPServerSocket(TEventBase* evb, size_t sz = 1500)
      : evb_(evb),
        packetSize_(sz),
        nextListener_(0) {
  }
   ...
   */
  void addListener(TEventBase* evb, Callback* callback) {
 ...
 private:
  ...
  void onReadError(const transport::TTransportException& ex) noexcept {
    LOG(ERROR) << ex.what();
