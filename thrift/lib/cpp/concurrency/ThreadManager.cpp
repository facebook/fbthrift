...
void ThreadManager::Impl::reportTaskStats(
  ...
  if (ThreadManager::Impl::observer_) {
    ...
    folly::RWSpinLock::ReadHolder g(ThreadManager::Impl::observerLock_);
    if (ThreadManager::Impl::observer_) {
      // Note: We are assuming the namePrefix_ does not change after the thread
      // is started.
      // TODO: enforce this.
      ThreadManager::Impl::observer_->addStats(namePrefix_,
                                                         queueBegin,
                                                         workBegin,
                                                         workEnd);
    }
...
Codel* ThreadManager::Impl::getCodel() {
  return &codel_;
}
  ...
  PriorityImpl(std::array<
    ...
    for (int i = 0; i < N_PRIORITIES; i++) {
      std::unique_ptr<ThreadManager> m(
        new ThreadManager::Impl(0, enableTaskStats, maxQueueLen));
...
PriorityThreadManager::newPriorityThreadManager(
  ...
  return shared_ptr<PriorityThreadManager>(
    new PriorityThreadManager::PriorityImpl(
      factories,
      enableTaskStats,
      maxQueueLen
    )
