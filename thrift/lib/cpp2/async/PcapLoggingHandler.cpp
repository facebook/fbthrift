 ...
 private:
  ...
  void maybeRotate() {
    ...
    if (fd_ == -1) {
      ...
      char buffer[100];
      strftime(buffer, sizeof(buffer), "_%Y%m%d-%H%M%S", &tmbuf);
      fd_ = open((prefix_ + buffer).c_str(),
...
std::shared_ptr<const PcapLoggingConfig> PcapLoggingConfig::config_ =
    std::make_shared<const PcapLoggingConfig>();
