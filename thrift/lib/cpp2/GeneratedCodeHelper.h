...
ret<F>
future_catching(F&& f) {
  using R = typename ret<F>::value_type;
  try { return f(); }
  catch(...) { return future_exn<R>(std::current_exception()); }
}

using CallbackBase = HandlerCallbackBase;
using CallbackBasePtr = std::unique_ptr<CallbackBase>;
template <class R> using Callback = HandlerCallback<R>;
...
template <class F>
void
async_tm(ServerInterface* si, CallbackPtr<fut_ret_drop<F>> callback, F&& f) {
  si->setEventBase(callback->getEventBase());
  si->setThreadManager(callback->getThreadManager());
  si->setConnectionContext(callback->getConnectionContext());
  auto fut = future_catching(std::forward<F>(f));
  auto callbackm = folly::makeMoveWrapper(std::move(callback));
  fut.then([=](folly::Try<fut_ret<F>>&& _return) {
      (*callbackm)->complete(std::move(_return));
  });
}
...
async_eb_oneway(ServerInterface* si, CallbackBasePtr callback, F&& f) {
  ...
  callbackp->runFuncInQueue([=]() mutable {
      si->setConnectionContext((*callbackm)->getConnectionContext());
      swallowing(fm.move());
  });
}
...
template <class F>
void
async_eb(ServerInterface* si, CallbackPtr<fut_ret_drop<F>> callback, F&& f) {
  si->setEventBase(callback->getEventBase());
  si->setThreadManager(callback->getThreadManager());
  auto callbackp = callback.get();
  auto callbackm = folly::makeMoveWrapper(std::move(callback));
  ...
  callbackp->runFuncInQueue([=]() mutable {
      si->setConnectionContext((*callbackm)->getConnectionContext());
      auto fut = future_catching(fm.move());
      fut.then([=](folly::Try<fut_ret<F>>&& _return) {
          (*callbackm)->complete(std::move(_return));
      });
  });
