...
public class DirectServerTask implements Runnable {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DirectServerTask.class);
  ...
  /**
   * If the queue is too long, we have to remove older tasks from it. The task we selected may not
   * belong to the selector thread. We use this to wakeup the corresponding selector thread to do
   * cleanup.
   */
  public void drop() {
    handler_.markError();
