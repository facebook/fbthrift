...
public class FramedTransportChannelHandlerFactory implements ChannelHandlerFactory {
  ...
  /**
   * Adjust the number of pooled transports. This should be called before any of the handler factory
   * is created.
   *
   * @param num the new number of pooled transports.
   */
   ...
   *
   * @param ch the socket channel that the handler is to work on
   * @param th the selector thread that manages this handler.
   * @return a new channel handler instance.
   */
  @Override
  public ChannelHandler newChannelHandler(SocketChannel ch, SelectorThread th) {
    return new FramedTransportChannelHandler(ch, th, pool_, server_.getProcessor());
  }
};
