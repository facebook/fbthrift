...
public class FramedTransportChannelHandler implements ChannelHandler {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(FramedTransportChannelHandler.class);
  ...
  FramedTransportChannelHandler(
      SocketChannel c, SelectorThread th, TDirectBufferTransport.Pool p, TProcessor tproc) {
  ...
  @Override
  public void wouldUseThreadPool(boolean useThreadPool) {
  ...
  @Override
  public void transition(SelectorThread s) {
    if (trans_ == null) {
    ...
    while (true) {
      switch (status_) {
          ...
          try {
            ...
            if (nreads < 0) {
              ...
              if (frameSize_ < 0) {
                status_ = STATUS_ERR;
                LOG.error(
                    "Negative frame size " + frameSize_ + " probably implies wrong buffer format");
                break;
              }
        ...
        case STATUS_PROCESSING:
          try {
            processor_.process(proto_, proto_, new DirectTConnectionContext(chan_, proto_));
          } catch (Exception e) {
