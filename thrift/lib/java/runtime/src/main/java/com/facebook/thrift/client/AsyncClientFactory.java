...
public class AsyncClientFactory extends AbstractRpcClientFactory {
  ...
  private static final String ASYNC_CLIENT_SUFFIX = "ReactiveAsyncWrapper";

  @Override
  <T> TypeConstructor getConstructor(Class<T> serviceClass) {
    try {
      Class<?> clientClass =
          Class.forName(serviceClass.getEnclosingClass().getName() + ASYNC_CLIENT_SUFFIX);
