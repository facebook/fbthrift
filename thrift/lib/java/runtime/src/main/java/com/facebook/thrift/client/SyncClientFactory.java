...
public class SyncClientFactory extends AbstractRpcClientFactory {
  public SyncClientFactory(
  ...
  private static final String SYNC_CLIENT_SUFFIX = "ReactiveBlockingWrapper";

  @Override
  <T> TypeConstructor getConstructor(Class<T> serviceClass) {
    try {
      Class<?> clientClass = Class.forName(serviceClass.getName() + SYNC_CLIENT_SUFFIX);
