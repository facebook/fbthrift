...
class TServerSocket {
  ...
  public function __construct($port=9090) {
    $this->host         = null;
    $this->port         = $port;
    $this->handle       = null;
  }
  ...
  public function listen() {
    $this->handle = stream_socket_server('tcp://0.0.0.0:'.$this->port,
                $errno, $errstr, STREAM_SERVER_BIND|STREAM_SERVER_LISTEN);
  }

  public function accept() {
    $client = stream_socket_accept($this->handle, -1);
    $result = new TSocket();
