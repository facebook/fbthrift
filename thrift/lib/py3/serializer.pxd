...
cdef extern from "<thrift/lib/cpp2/protocol/Serializer.h>" nogil:
    ...
    uint32_t CompactDeserialize "apache::thrift::CompactSerializer::deserialize"[T](const cIOBuf* buf, T& obj) except+
    void BinarySerialize "apache::thrift::BinarySerializer::serialize"[T](const T& obj, string* out) except+
    uint32_t BinaryDeserialize "apache::thrift::BinarySerializer::deserialize"[T](const cIOBuf* buf, T& obj) except+
    void JSONSerialize "apache::thrift::SimpleJSONSerializer::serialize"[T](const T& obj, string* out) except+
    uint32_t JSONDeserialize "apache::thrift::SimpleJSONSerializer::deserialize"[T](const cIOBuf* buf, T& obj) except+
