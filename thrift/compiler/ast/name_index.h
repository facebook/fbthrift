...
class name_index {
 ...
 public:
  ...
  // value or nullptr.
  //
  // `named` must be non-null and point to a t_named node that out lives
  // the name_index entry.
  ...
  // nullptr.
  //
  // Both `node` must be non-null and both 'node' and 'name' must reference to
  // values that out live the name_index entry. `name` must also not change
  // while in the name_index.
  // TODO(afuller): Except name via a std::string_view when c++17 can be used.
  const T* put(const std::string& name, const T* node);
  // Disallow temporary strings.
 ...
 private:
  // TODO(afuller): Use std::string_view instead of std::reference_wrapper<const
  // std::string> when available.
  std::unordered_map<
