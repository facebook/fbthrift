...
folly::Future<::std::int32_t> DerivedServiceSvIf::future_get_six() {
  ...
  return apache::thrift::detail::si::future(semifuture_get_six(), std::move(ka));
}
...
void DerivedServiceSvIf::async_tm_get_six(std::unique_ptr<apache::thrift::HandlerCallback<::std::int32_t>> callback) {
  ...
  switch (__fbthrift_invocation_get_six.load(std::memory_order_relaxed)) {
...
void DerivedServiceAsyncProcessor::getServiceMetadata(apache::thrift::metadata::ThriftServiceMetadataResponse& response) {
  ::apache::thrift::detail::md::ServiceMetadata<DerivedServiceSvIf>::gen(*response.metadata_ref(), *response.context_ref());
}
...
std::shared_ptr<folly::RequestContext> DerivedServiceAsyncProcessor::getBaseContextForRequest() {
