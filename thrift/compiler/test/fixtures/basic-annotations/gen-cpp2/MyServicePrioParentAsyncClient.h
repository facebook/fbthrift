...
class MyServicePrioParentAsyncClient : public apache::thrift::GeneratedAsyncClient {
 ...
 protected:
  void pongImpl(const apache::thrift::RpcOptions& rpcOptions, std::shared_ptr<apache::thrift::detail::ac::ClientRequestContext> ctx, apache::thrift::RequestClientCallback::Ptr callback);
 public:
  ...
  folly::coro::Task<void> co_pong(apache::thrift::RpcOptions& rpcOptions) {
    auto task = semifuture_pong(rpcOptions);
    const folly::CancellationToken& cancelToken =
        co_await folly::coro::co_current_cancellation_token;
    if (cancelToken.canBeCancelled()) {
      co_yield folly::coro::co_result(co_await folly::coro::co_awaitTry(folly::coro::detachOnCancel(std::move(task))));
