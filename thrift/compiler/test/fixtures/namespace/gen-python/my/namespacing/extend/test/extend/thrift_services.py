...
import py3lite_module_root.my.namespacing.test.hsmodule.thrift_services
import py3lite_module_root.my.namespacing.test.hsmodule.thrift_types

class ExtendTestServiceInterface(
    py3lite_module_root.my.namespacing.test.hsmodule.thrift_services.HsTestServiceInterface,
    metaclass=ABCMeta
):
    ...
    async def _fbthrift__handler_check(self, args: _fbthrift_iobuf.IOBuf, protocol: Protocol) -> _fbthrift_iobuf.IOBuf:
        args_struct = deserialize(py3lite_module_root.my.namespacing.extend.test.extend.thrift_types._fbthrift_ExtendTestService_check_args, args, protocol)
        value = await self.check(args_struct.struct1,)
        return_struct = py3lite_module_root.my.namespacing.extend.test.extend.thrift_types._fbthrift_ExtendTestService_check_result(success=value)
