...
instance Hashable.Hashable MyStruct where
  hashWithSalt salt record = salt   `Hashable.hashWithSalt` myStruct_MyIncludedField record   `Hashable.hashWithSalt` myStruct_MyIncludedInt record  
...
from_MyStruct record = Types.TStruct $ Map.fromList $ Maybe.catMaybes
  ...
  , (\_v3 -> Just (2, ("MyIncludedInt",Types.TI64 _v3))) $ myStruct_MyIncludedInt record
  ]
...
to_MyStruct (Types.TStruct fields) = MyStruct{
  ...
  myStruct_MyIncludedInt = maybe (myStruct_MyIncludedInt default_MyStruct) (\(_,_val5) -> (case _val5 of {Types.TI64 _val7 -> _val7; _ -> error "wrong type"})) (Map.lookup (2) fields)
  }
