...
cdef class UnEnumStruct(thrift.py3.types.Struct):
    ...
    def __copy__(UnEnumStruct self):
        cdef shared_ptr[cUnEnumStruct] cpp_obj = make_shared[cUnEnumStruct](
            deref(self._cpp_obj)
        )
        return UnEnumStruct.create(move_shared(cpp_obj))
...
cdef class List__Range:
    ...
    def index(self, item, start not None=__NOTSET, stop not None=__NOTSET):
        ...
        if not isinstance(item, Range):
            raise err
        cdef vector[cRange].iterator end = std_libcpp.prev(deref(self._cpp_obj).end(), <int64_t>offset_end)
...
cdef class List__Internship:
    ...
    def index(self, item, start not None=__NOTSET, stop not None=__NOTSET):
        ...
        if not isinstance(item, Internship):
            raise err
        cdef vector[cInternship].iterator end = std_libcpp.prev(deref(self._cpp_obj).end(), <int64_t>offset_end)
