...
cdef api void call_cy_Service_onStartServing(
    object self,
    cFollyPromise[cFollyUnit] cPromise
):
    cdef Promise_cFollyUnit __promise = Promise_cFollyUnit._fbthrift_create(cmove(cPromise))
    asyncio.get_event_loop().create_task(
        Service_onStartServing_coro(
            self,
            __promise
        )
    )
cdef api void call_cy_Service_onStopRequested(
    object self,
    cFollyPromise[cFollyUnit] cPromise
):
    cdef Promise_cFollyUnit __promise = Promise_cFollyUnit._fbthrift_create(cmove(cPromise))
    asyncio.get_event_loop().create_task(
        Service_onStopRequested_coro(
            self,
            __promise
        )
    )
...
async def Service_onStartServing_coro(
    object self,
...
async def Service_onStopRequested_coro(
    object self,
    Promise_cFollyUnit promise
):
    try:
