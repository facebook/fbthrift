#
# Autogenerated by Thrift
#
# DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
#  @generated
#

import thrift.py3.types
import thrift.py3.exceptions
from thrift.py3.types import NOTSET
from thrift.py3.serializer import Protocol
import typing as _typing

import sys
import itertools
from enum import Enum


class MyEnum(Enum):
    MyValue1: ...
    MyValue2: ...
    value: int


# Forward Definitions for Structs
class MyStruct(thrift.py3.types.Struct): ...
class MyDataItem(thrift.py3.types.Struct): ...


class MyStruct(thrift.py3.types.Struct):
    def __init__(
        self, *,
        MyIntField: int=None,
        MyStringField: str=None,
        MyDataField: MyDataItem=None
    ) -> None: ...

    def __call__(
        self, *,
        MyIntField: _typing.Union[int, NOTSET, None]=NOTSET,
        MyStringField: _typing.Union[str, NOTSET, None]=NOTSET,
        MyDataField: _typing.Union[MyDataItem, NOTSET, None]=NOTSET
    ): ...

    def __reduce__(self) -> _typing.Tuple[Callable, _typing.Tuple[_typing.Type[MyStruct], bytes]]: ...
    def __iter__(self) -> _typing.Iterator[_typing.Tuple[str, _typing.Any]]: ...
    def __bool__(self) -> bool: ...
    def __hash__(self) -> int: ...
    def __repr__(self) -> str: ...
    def __eq__(self, other: MyStruct) -> bool: ...
    def __lt__(self, other: MyStruct) -> bool: ...

    @property
    def MyIntField(self) -> int: ...
    @property
    def MyStringField(self) -> str: ...
    @property
    def MyDataField(self) -> MyDataItem: ...


class MyDataItem(thrift.py3.types.Struct):
    def __init__(
        self, *
    ) -> None: ...

    def __call__(
        self, *
    ): ...

    def __reduce__(self) -> _typing.Tuple[Callable, _typing.Tuple[_typing.Type[MyDataItem], bytes]]: ...
    def __iter__(self) -> _typing.Iterator[_typing.Tuple[str, _typing.Any]]: ...
    def __bool__(self) -> bool: ...
    def __hash__(self) -> int: ...
    def __repr__(self) -> str: ...
    def __eq__(self, other: MyDataItem) -> bool: ...
    def __lt__(self, other: MyDataItem) -> bool: ...



