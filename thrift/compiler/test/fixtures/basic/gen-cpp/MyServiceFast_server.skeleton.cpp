/**
 * Autogenerated by Thrift
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */

// This autogenerated skeleton file illustrates how to build a server.
// You should copy it to another filename to avoid overwriting it.

#include "thrift/compiler/test/fixtures/basic/gen-cpp/MyServiceFast.h"
#include <thrift/lib/cpp/async/TEventServer.h>
#include <thrift/lib/cpp/server/TConnectionContext.h>
#include <thrift/lib/cpp/util/TEventServerCreator.h>

using apache::thrift::TProcessor;
using apache::thrift::async::TEventServer;
using apache::thrift::util::TEventServerCreator;

using std::shared_ptr;

using namespace  ;

class MyServiceFastHandler : virtual public MyServiceFastIf {
 public:
  MyServiceFastHandler() {
    // Your initialization goes here
  }

  void ping() {
    // Your implementation goes here
    printf("ping\n");
  }

  void getRandomData(std::string& _return) {
    // Your implementation goes here
    printf("getRandomData\n");
  }

  bool hasDataById(int64_t id) {
    // Your implementation goes here
    printf("hasDataById\n");
    return bool();
  }

  void getDataById(std::string& _return, int64_t id) {
    // Your implementation goes here
    printf("getDataById\n");
  }

  void putDataById(int64_t id, const std::string& data) {
    // Your implementation goes here
    printf("putDataById\n");
  }

  void lobDataById(int64_t id, const std::string& data) {
    // Your implementation goes here
    printf("lobDataById\n");
  }

};

int main(int argc, char **argv) {
  int port = 9090;
  shared_ptr<MyServiceFastHandler> handler(new MyServiceFastHandler());
  shared_ptr<TProcessor> processor(new MyServiceFastProcessor(handler));
  TEventServerCreator serverCreator(processor, port);
  shared_ptr<TEventServer> server = serverCreator.createEventServer();
  server->serve();
  return 0;
}
