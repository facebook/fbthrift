...
// Add unit test for function isset_index in
// fbcode/thrift/compiler/lib/cpp2/util.h
TEST_F(UtilTest, get_isset_index) {
  auto program = t_program("path/to/program.thrift");
  auto s = t_struct(&program, "struct_name");
  auto i32 = t_base_type::t_i32();
  auto set = t_set(&i32);
  auto list = t_list(&i32);
  auto map = t_map(&i32, &t_base_type::t_double());
  std::unordered_map<t_field const*, int32_t> memo;

  s.append(std::make_unique<t_field>(&i32, "field1", 1));
  // Set field2 as required. And required field does not have isset
  s.append(std::make_unique<t_field>(&i32, "field2", 2));
  s.get_field(1)->set_qualifier(t_field_qualifier::required);
  s.append(std::make_unique<t_field>(&set, "field3", 3));
  // Set filed5 as required. And required field does not have isset
  s.append(std::make_unique<t_field>(&set, "field4", 4));
  s.get_field(3)->set_qualifier(t_field_qualifier::required);
  s.append(std::make_unique<t_field>(&map, "field5", 5));

  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(0)), 0);
  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(2)), 1);
  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(4)), 2);
}

TEST_F(UtilTest, get_isset_index2) {
  auto program = t_program("path/to/program.thrift");
  auto s = t_struct(&program, "struct_name");
  auto basic = t_struct(&program, "struct_name");
  auto i32 = t_base_type::t_i32();
  auto set = t_set(&i32);
  auto list = t_list(&i32);
  auto sring = t_base_type::t_string();
  auto map = t_map(&i32, &t_base_type::t_double());
  /*
  struct s {
  ...
  */
  basic.append(std::make_unique<t_field>(&sring, "b", 1));
  basic.append(std::make_unique<t_field>(&sring, "b_opt", 2));
  basic.get_field(1)->set_qualifier(t_field_qualifier::optional);
  basic.append(std::make_unique<t_field>(&sring, "b_req", 3));
  basic.get_field(2)->set_qualifier(t_field_qualifier::required);

  std::unordered_map<t_field const*, int32_t> memo;

  s.append(std::make_unique<t_field>(&basic, "a", 1));
  basic.get_field(0)->set_qualifier(t_field_qualifier::optional);
  s.get_field(0)->set_annotation("cpp.ref_type", "unique");
  s.append(std::make_unique<t_field>(&basic, "b", 2));
  basic.get_field(1)->set_qualifier(t_field_qualifier::optional);
  s.get_field(1)->set_annotation("cpp.ref_type", "unique");
  s.append(std::make_unique<t_field>(&sring, "c", 3));
  s.append(std::make_unique<t_field>(&sring, "d", 4));

  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(0)), -1);
  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(1)), -1);
  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(2)), 0);
  EXPECT_EQ(cpp2::isset_index(memo, s.get_field(3)), 1);
}
} // namespace
} // namespace apache::thrift::compiler
