...
mstch::map t_mstch_pyi_generator::extend_program(
  ...
  for (const auto included_program : program.get_includes()) {
    ...
    const auto ns = get_py_namespace(
      *included_program
    );
...
mstch::map t_mstch_pyi_generator::extend_service(
...
void t_mstch_pyi_generator::generate_services(const t_program& program) {
  ...
  for (const auto service : program.get_services()) {
    ...
    render_to_file(*service, template_, path / module);
  }
}
...
mstch::array t_mstch_pyi_generator::get_return_types(
  const t_program& program
) {
...
std::string t_mstch_pyi_generator::flatten_type_name(
  const t_type& type) const {
    if (type.is_list()) {
      return "List__" + flatten_type_name(
        *dynamic_cast<const t_list&>(type).get_elem_type()
      );
  } else if (type.is_set()) {
    return "Set__" + flatten_type_name(
