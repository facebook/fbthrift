...
void t_mstch_py3_generator::generate_structs(const t_program& program) {
  ...
  this->render_to_file(program, "types.pxd", path / name / (module + ".pxd"));
  this->render_to_file(program, "types.pyx", path / name / (module + ".pyx"));
}
...
void t_mstch_py3_generator::generate_services(const t_program& program) {
  ...
  auto name = program.get_name();
  this->render_to_file(program, "services.pxd", path / name / "services.pxd");
  this->render_to_file(program, "services.pyx", path / name / "services.pyx");

  std::string basename = "services_wrapper";
  auto cpp_path = boost::filesystem::path{name};
  this->render_to_file(
      program, "services_wrapper.h", cpp_path / (basename + ".h"));
  this->render_to_file(
...
void t_mstch_py3_generator::generate_clients(const t_program& program) {
  ...
  auto name = program.get_name();
  this->render_to_file(program, "clients.pxd", path / name / "clients.pxd");
  this->render_to_file(program, "clients.pyx", path / name / "clients.pyx");

  std::string basename = "clients_wrapper";
  auto cpp_path = boost::filesystem::path{name};
  this->render_to_file(
      program, "clients_wrapper.h", cpp_path / (basename + ".h"));
  this->render_to_file(
