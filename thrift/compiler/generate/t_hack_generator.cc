...
class t_hack_generator : public t_oop_generator {
 public:
    ...
    : t_oop_generator(program)
  {
    ...
    use_broken_hhvm_ext_ = option_is_specified(parsed_options, "brokenhhvm");
 ...
 private:
  ...
  /**
   * True if we want the broken hhvm behavior, where arrays are returned
   * instead of Maps/Vectors
   */
  bool use_broken_hhvm_ext_;
};
...
void t_hack_generator::generate_php_type_spec(ofstream& out,
  ...
  if (t->is_base_type() || t->is_enum()) {
    ...
    generate_php_type_spec(out, etype);
    indent(out) << ")," << endl;
    if (arraysets_ || use_broken_hhvm_ext_) {
      indent(out) << "'format' => 'array'," << endl;
    } else {
...
void t_hack_generator::generate_php_struct_writer(ofstream& out,
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    if (type->is_enum()) {
      t_enum* tenum = (t_enum*) type;
      out
        << " && " << php_namespace(tenum->get_program()) << tenum->get_name()
        << "::isValid($this->" << (*f_iter)->get_name() << ")";
    }
      ...
      if (type->is_base_type()) {
        ...
        out <<
          ...
          indent() << "  $" << name << " = " << php_namespace(tenum->get_program())
                   << tenum->get_name() << "::assertIsValid($" << name << ");"
                   << endl <<
          indent() << "}";
      }
      out << endl;
